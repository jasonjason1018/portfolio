...
// -------------------------登录页面---------------------------------------------------
	...
	$('#login').click(function(event) {
		...
		window.location.href ='/mySideProject_index'; // 页面跳转
	});
...
// ------------------------选择聊天室页面-----------------------------------------------
	...
	$('.theme img').click(function(event) {
		...
		$('body').css('background-image', 'url(side_project_assets/images/theme/' + theme_id + '_bg.jpg)'); // 设置背景
	});
...
// --------------------聊天室内页面----------------------------------------------------
	...
	$('.imgFileBtn').change(function(event) {
		...
		var str = '<img src="side_project_assets/images/chatimg/' + '1/201503/agafsdfeaef.jpg' +'" />'
	...
	$('#subxx').click(function(event) {
		...
		str = str.replace(/\>/g,'&gt;');
		str = str.replace(/\n/g,'<br/>');
		str = str.replace(/\[em_([0-9]*)\]/g,'<img src=side_project_assets/images"/face/$1.gif" alt="" />');
		if(str!='') {
...
// -----下边的代码不用管---------------------------------------
	...
	$('.clapboard').click(function(event) {
		...
		$('.user_portrait img').attr('src', 'side_project_assets/images/user/' + $('.user_portrait img').attr('ptimg') + '.png');
	...
	$('.select_portrait img').hover(function() {
		...
		$('.user_portrait img').attr('src', 'side_project_assets/images/user/' + portrait_id + '.png');
	}, function() {
		var t_id = $('.user_portrait img').attr('portrait_id');
		$('.user_portrait img').attr('src', 'side_project_assets/images/user/' + t_id + '.png');
	});
	...
	function sends_message (userName, userPortrait, message) {
		if(message!='') {
			$('.main .chat_info').html($('.main .chat_info').html() + '<li class="right"><img src="side_project_assets/images/user/' + userPortrait + '.png" alt=""><b>' + userName + '</b><i>09:15</i><div class="aaa">' + message  +'</div></li>');
		}
	}
